window.TS = (() => {
  ...
  function displayEvents() { return state.data.events.map((e) => ({ ...e, status: lifecycle(e) })); }
  function ago(iso) {
  ...
  function deriveSystem(system, events) {
    ...
    const active = events.filter((e) => e.system_id === system.id && e.status === "active" && (!e.last_verified_at || Date.now() - new Date(e.last_verified_at).getTime() <= freshnessMs))
  ...
  return { state, $, $$, FLAGS, SEVERITY_ORDER, SIGNAL_LABELS, TYPE_LABELS, lifecycle, displayEvents, ago, shortDate, localRange, formatTime, labelEventType, deriveSystem, capitalize, escapeHtml, safeUrl };
})();
